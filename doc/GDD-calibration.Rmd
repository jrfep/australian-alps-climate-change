...
dts.alp <- values(alp.GDD)[q1[!is.na(q1)],]
...
d1 <- BOM.GDD %>% filter(stn_num %in% wst.alp@data$stn_num) %>% dcast(stn_num~year,value.var="GDD")
yy <- as.numeric(colnames(d1)[-1])
matplot(yy,t(d1[,-1]),type='l',lty=1,lwd=2,xlim=c(1970,2020))
matlines(1979:2013,t(dts.alp),lty=2,lwd=2)
