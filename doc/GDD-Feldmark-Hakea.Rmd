...
for (k in 1:nrow(gdd.eco)) {
   ...
   IV <- predict(mdl,data.frame(x=2000))
   FV <- predict(mdl,data.frame(x=2050))
   eco.RS <- rbind(eco.RS,data.frame(k,RS=(FV-CT)/(IV-CT)))
}
summary(eco.RS$RS)
```

In this case, relative severity is > 30% for all localities (>80% extent), thus the resulting category would be VU.
