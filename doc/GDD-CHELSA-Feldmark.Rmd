...
get.map.cells <- function(locs,map) {
   s1 <- cellFromXY(map,get(locs))
   v1 <- unique(s1[!is.na(s1)])
   res <- data.frame(points=rep(locs,length(v1)),
      cell=v1)
   return(res)
}
...
eco.st <- st_transform(st_read(arch),crs = "+proj=utm +zone=55 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs ")
```
...
eco.xy <- st_coordinates(st_centroid(eco.st))
##eco.polygons <- st_geometry(eco.st)

```

# Invasive species

Species that can invade Feldmark (data from ALA):

```{r invasive-species, include=TRUE}
...
Some species have recent detections near the feldmark:

```{r expansion-of-species}


Celmisia_longifolia.xy$yy <- (Celmisia_longifolia.xy$year - min(Celmisia_longifolia.xy$year,na.rm=T))/ (max(Celmisia_longifolia.xy$year,na.rm=T) - min(Celmisia_longifolia.xy$year,na.rm=T))
plot(st_geometry(eco.st),border="darkgreen")
points(Celmisia_longifolia.xy,cex=Celmisia_longifolia.xy$yy,pch=19,col=rgb(.3,.1,.6,.5))
...
dts <- values(alp.GDD)[qs$cell,]
dts.alp <- cbind(qs,dts)
